...
const AlertManagement = () => {
  ...
  const handleStatusChange = async (id: string, newStatus: AlertStatus) => {
    try {
      const res = await api.patch(`/alerts/${id}`, { status: newStatus });
  ...
  const handleEditSave = async () => {
    ...
    try {
      const res = await api.patch(`/alerts/${editingAlert.id}`, {
      ...
      <div className="rounded-xl border border-border bg-card overflow-hidden shadow-sm">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={6} className="px-6 py-10 text-center text-muted-foreground">Loading alerts...</td></tr>
              ) : filtered.length === 0 ? (
                <tr><td colSpan={6} className="px-6 py-10 text-center text-muted-foreground">No alerts found.</td></tr>
              ) : filtered.map(a => (
