...
const OfficerAlerts = () => {
  ...
  const handleSaveNote = async (id: string) => {
    if (!note.trim()) return;
    try {
      const res = await api.patch(`/alerts/${id}`, { notes: note });
      setAlerts(prev => prev.map(a => a.id === id ? res.data : a));
  ...
  if (loading) return <div className="py-12 text-center text-muted-foreground">Loading alerts...</div>;
            ...
            <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
              <div className="space-y-1">
                <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-primary">
                  ...
                  INCIDENT #{a.id.toUpperCase().slice(0, 8)}
                </div>
